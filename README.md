...
`BMG_WORKSPACE_IDLE_TIMEOUT_SECONDS` controls automatic workspace cleanup and defaults to `1800` (30 minutes). Any browser tool activity refreshes the idle deadline. When the deadline expires, BMG re-checks the exact tracked workspace window, closes every tab in that window only, clears the workspace state, and creates a fresh workspace on the next page-directed call. Set the value to `0` to disable idle cleanup. This closes tabs but intentionally preserves the shared Edge profile, cookies, and login state.
